...
// uinput and kernel-module mouse backends. See vmouse.hpp for why the
// linux/uinput.h include lives here and not in the header.
#include "vmouse.hpp"
...
bool VMouse::open() {
    ...
    // REL_X/REL_Y are ignored entirely. Declared but never emitted by the
    // aim path - only press() uses it.
...
void VMouse::moveRel(double dx, double dy) {
...
bool VMouse::fail(const char* what) {
    printf("[vmouse] ioctl %s failed: %s\n", what, strerror(errno));
    ::close(fd_); fd_ = -1;
    return false;
}

void VMouse::emit(unsigned short type, unsigned short code, int value) {
